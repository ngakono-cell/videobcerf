/**
 * CreateType2Screen – Admin standalone screen to create a new Type 2 (Agent) account.
 * (Functionality also embedded in AdminDashboard → Créer tab)
 */
...
export default function CreateType2Screen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const { error: fnError } = await supabase.functions.invoke('create-type2', {
        method: 'POST',
      ...
      setSuccess(`Compte agent "${username}" créé avec succès.`);
    ...
    <KeyboardAvoidingView behavior={process.env.EXPO_OS === 'ios' ? 'padding' : 'height'} className="flex-1 bg-background">
      ...
      <View className="bg-primary px-6 pt-14 pb-5">
        <View className="flex-row items-center gap-3">
          ...
          <View>
            <Text className="text-primary-foreground text-xl font-bold">Créer un agent</Text>
            <Text className="text-primary-foreground text-sm opacity-70">Nouveau compte Type 2</Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView contentContainerClassName="px-4 py-5 pb-10" keyboardShouldPersistTaps="handled">
        <View className="bg-card rounded-2xl p-4 gap-4"
          ...
          {[
            ...
            { label: 'Identifiant', value: username,  setter: setUsername,  placeholder: 'agent_dupont', cap: 'none' as const,    kbd: 'default' as const },
          ...
          <Pressable onPress={handleCreate} disabled={loading} className="bg-primary rounded-xl py-4 items-center"
            ...
            {loading ? <ActivityIndicator color="#fff" /> : (
              <Text className="text-primary-foreground font-semibold text-base">Créer le compte agent</Text>
            )}
          </Pressable>
        </View>
        ...
        <View className="bg-secondary rounded-2xl p-4 mt-4" style={{ borderCurve: 'continuous' } as object}>
          ...
          <Text className="text-muted-foreground text-xs" style={{ lineHeight: 18 }}>
            Les comptes agents (Type 2) peuvent recevoir des demandes de certification, visualiser les pièces d'identité et certifier les numéros.
            Communiquez l'identifiant et le mot de passe à l'agent de manière sécurisée.
          </Text>
        </View>
      </ScrollView>
